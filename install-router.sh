...
sed -i "s|ROUTER_ID|${ROUTER_ID}|g" router.json
sed -i "s|\${OPENSHIFT_MASTER}|${MASTER_URL}|"       router.json
sed -i "s|\${OPENSHIFT_INSECURE}|${OPENSHIFT_INSECURE}|"   router.json
sed -i "s|\${OPENSHIFT_CA_DATA}|${OPENSHIFT_CA_DATA}|"     router.json
...
if [ "${OPENSHIFT}" == "" ]; then
    ...
    "${OPENSHIFT}" --server="${MASTER_URL}" create -f router.json
fi
